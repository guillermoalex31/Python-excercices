...
api_key = ""  # O usa os.getenv("OPENAI_API_KEY") para extraer de variables de entorno
...
def generar_datos_sinteticos(prompt_base, cantidad_datos):
    ...
    for _ in range(cantidad_datos):
        ...
        prompt = fr'''Eres un experto en desarrollo personal y profesional con un enfoque en habilidades blandas. Tu tarea es proporcionar un plan de mejora personalizado basado en los siguientes datos, que incluyen las puntuaciones históricas, fechas, facetas y resúmenes del comportamiento.
...
Basado en los datos anteriores, genera un plan de mejora personalizado para cada subcategoría. Aquí tienes un ejemplo:
...
# Ejemplo de uso
prompt_base = "Estoy buscando generar datos sintéticos para un proyecto de análisis de datos."
